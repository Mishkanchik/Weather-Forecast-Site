    ...
    <svg viewBox="0 0 48 48" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.g animate={{ rotate: 360 }} transition={{ duration: 20, ease: "linear", repeat: Infinity }}
        ...
        {[0, 60, 120, 180, 240, 300].map((deg) => (
          <motion.line key={deg} x1="16" y1="6" x2="16" y2="9" stroke="#FBBF24" strokeWidth={1.5} strokeLinecap="round"
    ...
    <svg viewBox="0 0 48 48" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      ...
      <motion.g animate={{ y: [4, 0] }} transition={{ duration: 3, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }}>
        ...
        {[0, 30, 60, 90, 120, 150, 180].map((deg) => (
          <motion.line key={deg} x1="24" y1="14" x2="24" y2="17" stroke="#FBBF24" strokeWidth={1.5} strokeLinecap="round"
...
export function AnimatedWeatherIconsDemo() {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 p-4">
      {ALL_ICONS.map(({ name, Icon }) => (
        <div key={name} className="flex flex-col items-center gap-2 p-4 border rounded-xl bg-card transition-all hover:shadow-md hover:scale-105 group cursor-pointer">
          <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800 transition-colors group-hover:bg-slate-100 dark:group-hover:bg-slate-700">
            <Icon size={48} />
          </div>
